...
export function DashboardPageClient() {
  const dashboardState = useAppDataStore((state) => state.dashboard);
  const shell = useAppDataStore((state) => state.shell);
  const startLoading = useAppDataStore((state) => state.startLoading);
  const setDashboard = useAppDataStore((state) => state.setDashboard);
    ...
    void fetch("/api/dashboard/summary")
      .then(async (response) => {
        ...
        if (!response.ok || !payload.ok) {
          throw new Error(payload.error ?? "Không thể tải dashboard.");
        }

        setDashboard(payload.summary as DashboardSummary);
      })
      .catch((error) => {
        setError(
          "dashboard",
  ...
  }, [dashboardState.loaded, dashboardState.loading, setDashboard, setError, startLoading]);

  if (dashboardState.error) {
    return (
      <SurfaceCard>
        <p className="text-sm text-[var(--danger)]">{dashboardState.error}</p>
      </SurfaceCard>
    );
  }
  ...
  const walletBalance = shell.credits || summary.walletBalance;
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        description="Màn hình điều phối prompt, image, video, credits và các dự án đang chạy trong cùng một workspace."
      />
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        {summary.stats.map((item, index) => (
          <SurfaceCard key={item.label} className="p-[14px]">
            ...
            <p className={`mt-2 text-[10px] ${item.tone === "success" ? "text-[var(--success)]" : "text-[var(--pending)]"}`}>
              {item.note}
            </p>
          </SurfaceCard>
        ))}
        ...
        <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-5">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <DashboardSelect label="Chọn model" value="Gemini / Veo / GPT" />
              <DashboardSelect label="Chọn tỷ lệ" value="9:16 · 1:1 · 16:9 · 4:3 · 3:4" />
              <DashboardSelect label="Upload tham chiếu" value="Ảnh, packshot, frame đầu" />
            </div>

            <div className="flex flex-col gap-3 border-t border-[var(--border)] pt-4 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-[var(--muted-foreground)]">
                Ước tính: <span className="text-[var(--heading)]">{summary.workflowEstimate}</span>
              </p>
          ...
          <div className="space-y-4 rounded-[12px] border bg-[var(--surface-muted)] p-4">
            ...
            {[
              `${summary.projectCount} dự án trong workspace`,
              `${walletBalance} credits còn lại cho job tạo ảnh và video`,
              `${summary.recentOutputs.length} output gần đây đã được ghi lịch sử`,
              "OpenAI, Google và 9Router được map theo từng tác vụ",
            ].map((item) => (
              <div
                key={item}
      ...
      <div className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
        <RecentOutputs outputs={summary.recentOutputs} />
        <RecentProjects summary={summary} />
      </div>
  ...
  return (
    <div className="space-y-5 animate-pulse">
      <div className="space-y-3">
        <div className="h-3 w-36 rounded-full bg-[var(--surface-muted)]" />
      ...
      <SurfaceCard className="h-80 rounded-[var(--radius-shell)]" />
    </div>
  );
}

function RecentOutputs({ outputs }: { outputs: QuickGenerationRecord[] }) {
  return (
    <SurfaceCard>
      <div className="flex items-center justify-between gap-4">
        <div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {outputs.length > 0 ? (
          outputs.map((item) => (
            ...
            >
              <div
                className="flex aspect-[4/3] items-center justify-center border-b bg-cover bg-center text-[var(--muted)]"
                style={item.output_url ? { backgroundImage: `url(${item.output_url})` } : undefined}
              >
                {!item.output_url && <OutputIcon type={item.type} />}
              </div>
              <div className="space-y-2 p-3">
                ...
                <p className="line-clamp-2 text-xs text-[var(--muted-foreground)]">{item.model}</p>
...
function RecentProjects({ summary }: { summary: DashboardSummary }) {
  return (
    <SurfaceCard>
      <div className="flex items-center justify-between gap-4">
        <div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3">
        {summary.recentProjects.length > 0 ? (
          ...
          <div className="rounded-[12px] border border-dashed px-4 py-10 text-center text-sm text-[var(--muted-foreground)] md:col-span-3">
            Chưa có dự án nào.
          </div>
        )}
      </div>
...
function ActivityList({ summary }: { summary: DashboardSummary }) {
  return (
...
function StatIcon({ index }: { index: number }) {
  const icons = [
    <Clapperboard key="video" className="h-4 w-4 text-[var(--highlight)]" />,
    <ImageIcon key="image" className="h-4 w-4 text-[var(--highlight)]" />,
    <Sparkles key="prompt" className="h-4 w-4 text-[var(--highlight)]" />,
...
function OutputIcon({ type }: { type: QuickGenerationRecord["type"] }) {
  if (type === "video") {
    return <CirclePlay className="h-8 w-8 text-[var(--highlight)]" />;
  }

  if (type === "image") {
    return <ImageIcon className="h-8 w-8 text-[var(--highlight)]" />;
  }

  return <Sparkles className="h-8 w-8 text-[var(--highlight)]" />;
}
