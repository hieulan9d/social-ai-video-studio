...
export function ProjectPreviewCanvas({
  ...
  return (
    <ProjectPreviewContext.Provider value={{ onAssetClick: handleAssetClick }}>
            ...
            {preview && (
              <>
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-[8px] border border-[var(--border)] px-3 py-2 text-xs text-[var(--muted-foreground)] hover:text-[var(--foreground)] transition"
                >
      ...
      {/* Children (tab content with clickable assets) */}
      {children}
    </ProjectPreviewContext.Provider>
  );
}

/* Context to pass click handler down to asset grids */
...
/* Clickable asset grid */
export function ClickableAssetGrid({
  title,
  assets,
}: {
  title: string;
          ...
          {assets.map((asset) => {
            ...
            return (
              <button
                ...
                className="group rounded-[12px] border bg-[var(--surface)] p-2 text-left transition hover:border-[var(--accent)] hover:ring-1 hover:ring-[var(--accent)]"
              >
                {isVideo ? (
                  <video
                    src={url}
                ...
                <p className="mt-2 line-clamp-2 text-xs text-[var(--muted-foreground)] group-hover:text-[var(--foreground)]">
