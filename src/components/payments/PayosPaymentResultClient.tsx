...
export function PayosPaymentResultClient({ orderId }: { orderId: string | null }) {
  const { refreshCredits } = useCredits();
  const [message, setMessage] = useState(
    orderId
      ? "Chúng tôi đang xác nhận thanh toán từ PayOS."
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      ...
      if (attempts >= 20) {
        window.clearInterval(timer);
        setMessage(
          "Chúng tôi đang xác nhận thanh toán từ PayOS. Credit sẽ được cộng tự động sau khi hệ thống nhận được webhook.",
        );
      }
    }, 3000);
